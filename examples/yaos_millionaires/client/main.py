...
def richest(session: requests.Session, url: str, pk: bytes, sk: bytes) -> str:
    ...
    logging.info("Response sucessfully decrypted")
...
def verify(url: str) -> tuple[Path, bytes, bytes]:
    ...
    logging.info("SGX remote attestation from MAA successful")
    logging.info(f"RA-TLS certificate written to {ratls_cert_path}")
