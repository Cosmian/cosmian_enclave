...
SECRETS = json.loads(Path(os.environ["SECRETS_PATH"]).read_text(encoding="utf-8"))
...
def participants() -> Response:
    ...
    if "participants" not in SECRETS:
        app.logger.error("no participants found")
        return jsonify({"participants": None})

    return jsonify({"participants": SECRETS["participants"]})
